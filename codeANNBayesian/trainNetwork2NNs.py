...
X = np.genfromtxt(os.path.abspath(os.path.join(__file__ ,"../../Jiang3CasesDataset/0.1Percent")) + "/" + "sampleX.txt")
Xp = np.genfromtxt(os.path.abspath(os.path.join(__file__ ,"../../Jiang3CasesDataset/0.1Percent")) + "/" + "sampleXp.txt")
Xp = np.reshape(Xp, (Xp.shape[0],1))
Y = np.genfromtxt(os.path.abspath(os.path.join(__file__ ,"../../Jiang3CasesDataset/0.1Percent")) + "/" + "sampleY.txt")
...
mup = center(Xp, settings["centering_method"])
sigmap = scale(Xp, settings["scaling_method"])
scaled_Xp = center_scale(Xp, mup, sigmap)
...
newDirName =   "optimizeNetwork2NNs" + "_epochs=" + str(settings["iterations_optimizer"]) + "_evals=" + str(settings["network_epochs"]) + "scaling=" + str(settings["scaling_method"])
os.mkdir(newDirName)
os.chdir(newDirName)
...
def create_model(layers, neurons, activation, alpha):
    ...
    X_train, X_test, Xp_train, Xp_test, y_train, y_test = train_test_split(scaled_X, scaled_Xp, Y, test_size=test_ratio, random_state=42)    
   
    #model = Sequential()
    # the first branch opreates on the second input   
    inputX = Input(shape = (X_train.shape[-1],)) 
    #print("X_train.shape[-1]=",X_train.shape[-1])
    # Add all the hidden layers and activate each of them
    for i in range(layers+1):
        if i == 0:
            #model.add(Dense(neurons, kernel_initializer='normal', input_shape = (X_train.shape[-1],)))
            #model.add(Activation(str(activation)))
            layerX = Dense(neurons, kernel_initializer='normal', activation=tf.keras.activations.get(activation))(inputX)
        #model.add(Dense(neurons, kernel_initializer='normal'))
        #model.add(Activation(str(activation)))
        layerX = Dense(neurons, kernel_initializer='normal', activation=tf.keras.activations.get(activation))(layerX)
    layerX = Dense(1, kernel_initializer='normal', activation=tf.keras.activations.get(activation))(layerX)
    layerX = Model(inputs=inputX, outputs=layerX)
    
    #inputXp = Input(shape = (Xp_train.shape[-1],))
    inputXp = Input(shape = (X_train.shape[-1],))
    #print("Xp_train.shape[-1]=",Xp_train.shape[-1]) 
    # Add all the hidden layers and activate each of them
    for i in range(layers+1):
        if i == 0:
            #model.add(Dense(neurons, kernel_initializer='normal', input_shape = (X_train.shape[-1],)))
            #model.add(Activation(str(activation)))
            layerXp = Dense(neurons, kernel_initializer='normal', activation=tf.keras.activations.get(activation))(inputXp)
        #model.add(Dense(neurons, kernel_initializer='normal'))
        #model.add(Activation(str(activation)))
        layerXp = Dense(neurons, kernel_initializer='normal', activation=tf.keras.activations.get(activation))(layerXp)
    layerXp = Dense(1, kernel_initializer='normal', activation=tf.keras.activations.get(activation))(layerXp)
    layerXp = Model(inputs=inputXp, outputs=layerXp)    

    # 3rd Input: Pressure gradient 
    gradPInput = Input(shape = (Xp_train.shape[-1],))
    gradPInputWeighted = Multiply()([layerXp.output, gradPInput])
    
    # Combine all networks
    combined = Concatenate()([layerX.output, gradPInputWeighted])
    
    # Add the output layer (no activation needed, since it is for prediction)    
    outputY = Dense(Y.shape[1])(combined)
    #outputY = Dense(2, activation="relu")(combined)
    #outputY = Dense(1, activation="linear")(outputY)
    model = Model(inputs=[layerX.input, layerXp.input, gradPInput], outputs=outputY)
    #model = Model(inputs=layerX.input, outputs=outputY)
    ...
    return model, X_train, y_train, X_test, y_test, Xp_train, Xp_test
...
def fitness(layers, neurons, activation, alpha):
    ...
    try:
        ...
        model, X_train, y_train, X_test, y_test, Xp_train, Xp_test = create_model(layers=layers, neurons=neurons, activation=activation, alpha=alpha)
        ...
        history = model.fit(x=[X_train, X_train, Xp_train], y=y_train, validation_data=([X_test, X_test, Xp_test],y_test), epochs=int(settings["network_epochs"]), batch_size=batchS, callbacks=[callback_log, earlyStopping])
...
pred_all = best_model.predict([scaled_X, scaled_X, scaled_Xp])
...
if settings["plot_results"]:
    for ii in range(Y.shape[1]):
        ...
        #_AO lgnd.legendHandles[0]._sizes = [45]
        lgnd.legend_handles[0]._sizes = [45]
        #_AO lgnd.legendHandles[1]._sizes = [45]
        lgnd.legend_handles[1]._sizes = [45]
